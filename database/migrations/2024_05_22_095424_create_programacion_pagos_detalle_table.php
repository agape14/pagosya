...
class CreateProgramacionPagosDetalleTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('programacion_pagos_detalle', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('id_programacion');
            $table->string('descripcion', 100);
            $table->unsignedBigInteger('id_concepto');
            $table->decimal('monto', 10, 2);
            $table->foreign('id_programacion')->references('id')->on('programacion_pagos')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('programacion_pagos_detalle');
    }
}
